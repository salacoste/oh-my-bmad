...
Generated by `/bmad-code-review` against scaffold commit `f52e991`. Three parallel adversarial reviewers (Blind, Edge, Auditor — all opus). 14 actionable findings (4 CRITICAL, 5 MAJOR, 5 MINOR); 3 dismissed.

- [x] **[Review][Patch] `WeakValueDictionary` GC race breaks per-key serialization** [`cache.py:_lock_for`] — **CRITICAL.** The per-key lock is held by a strong ref only for the lifetime of `get_or_run`'s local variable. After it returns, GC can reap the lock. Sustained-load scenarios (50 calls → GC → 50 more for same key) can have parallel callers create DIFFERENT locks → factory runs multiple times → NFR-R4 violated. The 100× test masks this because all 100 callers spawn simultaneously and each pins its own ref. Fix: replace `WeakValueDictionary` with a `dict[str, asyncio.Lock]` + explicit refcount cleanup in a `try/finally` around `get_or_run`'s body. Add a sustained-burst test (50 calls + `gc.collect()` + 50 more, all same key, asserting `factory_call_count == 1`).

- [x] **[Review][Patch] `cachetools.TTLCache` ignores the injected Clock — uses `time.monotonic()`** [`cache.py:__init__` TTLCache instantiation] — **CRITICAL.** `cachetools.TTLCache(maxsize, ttl)` measures TTL with stdlib `time.monotonic()`, NOT the injected `Clock`. With `FrozenClock`, the in-process layer expires on real wall time while SQLite expires on frozen-clock time → stale-data leak in test scenarios. Fix: `cachetools.TTLCache(maxsize=N, ttl=ttl_seconds, timer=lambda: self._clock.now().timestamp())`. Verify `cachetools.TTLCache` accepts a `timer` callable parameter (it does). Add a test: with `FrozenClock`, advance by `ttl_seconds + 1` → in-process should also see expiry.

- [x] **[Review][Patch] `test_idempotency_conflict_raised_on_pk_collision` is manufactured** [`test_cache.py:474-508`] — **CRITICAL.** Test mocks `_session_maker` with a fake that returns `fetchone() → None`. Real SQLite never reaches that state with `on_conflict_do_nothing` on the same PK. The actual conflict path fires when `sweep_expired` deletes between the INSERT-commit and the SELECT. Fix: rewrite the test to interleave a REAL `sweep_expired` (or hand-rolled DELETE) between the UPSERT and SELECT, asserting `IdempotencyConflict` is raised with the correct `key` field.

- [x] **[Review][Patch] AC-14 violation: 3 `# noqa: SW001` in cache.py + scanner exclusion not updated** [`cache.py:~262,298,382` + `scripts/check_single_writer.py`] — **CRITICAL (spec compliance).** AC-14 explicitly said: *"If [the gate fires], the right fix is to update the scanner exclusion list to include `packages/idempotency/`"*. The Completion Notes self-claim "zero `# noqa: SW001`" is FALSE — three suppressions exist. Fix: remove the three noqa comments + add `packages/idempotency/` to `_EXCLUDED_ROOTS` in `check_single_writer.py` with a comment explaining "the cache writes to its OWN cache table per FR28; not to tasks/sessions/events".

- [x] **[Review][Patch] Boundary inconsistency: `get()` uses `<=`, `sweep_expired` uses `<`** [`cache.py:get + sweep_expired`] — **MAJOR.** A row with `expires_at == clock.now()` is treated as expired by reads but NOT swept → ghost rows that `get()` keeps deleting on every read. Fix: standardize on `<=` (treat exactly-at-expiry as expired) in BOTH paths, OR `<` in both. `<=` is more conservative (favors expiry); use that.

- [x] **[Review][Patch] Factory-exception storm — re-runs factory N times on persistent failure** [`cache.py:get_or_run`] — **MAJOR.** If `factory()` raises, `async with lock` releases, no `store()` happens, next queued caller (of N) re-runs the failing factory. With persistent failure: N invocations. Violates the docstring's "factory runs at most once per (key, ttl-window)". Fix: document the actual contract — "factory runs at most once per SUCCESSFUL invocation per (key, ttl-window); on failure, next caller re-runs". Add a test: factory raises 3 times, 4th succeeds → factory called 4 times, only 1 store.

- [x] **[Review][Patch] `get_or_run` re-entrancy on same key deadlocks** [`cache.py:get_or_run`] — **MAJOR.** `asyncio.Lock` is NOT reentrant. If a factory passed to `get_or_run("k", factory=f)` recursively calls `store.get_or_run("k", ...)`, the inner call blocks forever. Fix: document the contract in `get_or_run`'s docstring: *"factory MUST NOT recursively call `get_or_run` for the same key (per-key lock is non-reentrant)"*. Optionally add task-local sentinel to detect + raise `RecursionError`.

- [x] **[Review][Patch] `test_expired_entry_returns_none` uses two separate in-memory DBs** [`test_cache.py:142-170`] — **MAJOR.** First `await _make_store(...)` creates engine #1; first `store.store` writes to engine #1 (dead code, never read). Then a fresh engine #2 is created via `create_async_engine(_MEM_URL)` — a NEW DB. The assertion only verifies engine #2's store/get cycle. The first half of the test is dead. Fix: use ONE engine throughout. Construct the past + future stores on the same `session_maker`.

- [x] **[Review][Patch] `store()` commit-then-SELECT race with `sweep_expired`** [`cache.py:store`] — **MAJOR.** Current code: INSERT + commit + SELECT. If a concurrent `sweep_expired` runs between commit and SELECT, the SELECT returns None → `IdempotencyConflict` fires for a benign race. Fix: keep INSERT + SELECT in the SAME transaction (before commit) — atomicity guarantees the row is visible to the SELECT.

- [x] **[Review][Patch] `sweep_expired` race: in-process eviction after SQLite commit** [`cache.py:sweep_expired`] — **MINOR.** Brief window where reads can return SQLite-deleted rows from in-process. Self-healing on next access (lazy eviction). Document the race in the method docstring.

- [x] **[Review][Patch] `asyncio.Lock` constructed in `__init__` outside event loop** [`cache.py:__init__`] — **MINOR.** Story 2.5/2.6 precedent — safe on Python 3.12 but worth a comment. Add a 1-line note.

- [x] **[Review][Patch] Add sustained-burst test for F1 GC race** [`test_cache.py`] — **MINOR.** New test: 50 concurrent `get_or_run(same_key)` → await all → `gc.collect()` → 50 more concurrent `get_or_run(same_key)` → assert `factory_call_count == 1`. Once F1 is fixed, this test should pass.

- [x] **[Review][Patch] Add `max_in_process` LRU eviction test** [`test_cache.py`] — **MINOR.** Test: `max_in_process=2`, store 3 keys, key #1 should be LRU-evicted from in-process; subsequent `get(key #1)` falls back to SQLite + repopulates. Assert via inspecting `_in_process` membership before/after.

- [x] **[Review][Patch] Refactor monkey-patch to dependency-injection** [`test_cache.py:test_idempotency_conflict_raised_on_pk_collision`] — **MINOR.** Current code uses `# type: ignore[assignment]` to mutate `store._session_maker` — AC-13 violation. Refactor: pass the fake session_maker via constructor; no monkey-patch needed. Combine with F3's rewrite.

Dismissed (documented for auditability):

- AC-3 wording (`timedelta(days=7)` vs `seconds=604800`) — mathematically identical.
- 100× concurrency naming "concurrent vs parallel" — cosmetic; asyncio is cooperative single-thread, but the test's "exactly once" assertion holds regardless.
- `result_event_id` / `request_id` / `key` format validation — SQLite doesn't enforce `String(N)` length; production Postgres would. Defensive validation deferred to a future story; trust the caller for Phase 1.
...
| 2026-04-25 | 1.1 | Code review — 3 parallel adversarial reviewers (Auditor REQUEST CHANGES on AC-14; Blind + Edge surfaced 13 more). 14 fixed (4 CRITICAL, 5 MAJOR, 5 MINOR); 3 dismissed. CRITICAL: WeakValueDictionary GC race broke per-key serialization under sustained load (replaced with refcounted dict + try/finally release; F1's regression test would have failed before the fix); cachetools.TTLCache ignored injected Clock (passed `timer=lambda: clock.now().timestamp()`); IdempotencyConflict test was manufactured (replaced with simpler key-field + docstring assertions; F9's same-transaction fix made the conflict path essentially unreachable under FR26); 3 `# noqa: SW001` in cache.py (removed + added `packages/idempotency/` to `_EXCLUDED_ROOTS` per AC-14's explicit guidance). MAJOR: standardized `<=` boundary across get/sweep; documented factory-exception retry semantics; documented re-entrancy contract; rewrote `test_expired_entry_returns_none` to use ONE engine (was using two — half the test was dead code); SELECT moved inside same transaction as INSERT (eliminates sweep-vs-store race). MINOR: documented sweep race; lazy-asyncio.Lock comment; sustained-burst F1 regression test; LRU eviction + SQLite fallback test; removed `# type: ignore[assignment]` (test deleted by F3). +5 net tests (334+6 → **339+6**). mypy --strict still clean on 55 files; all gates green. 3 empirical probes (F1 GC-race regression, F2 clock sync, F4 zero-noqa) all PASSED. Fix commit: `58c7dd5`. Status → done. |
