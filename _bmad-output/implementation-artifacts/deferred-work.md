...
- **D6 — PH-A7c synthetic trace_id forensics column** (pass-1 PH-A7 follow-up): Migrator back-fill currently sets `trace_id = request_id` for pre-1.1.0 envelopes without distinguishing synthetic from real. Operators inspecting `/trace` cannot tell which causal-chain links were inferred vs originated from an operator command. Add a separate `trace_id_synthetic_source: str | None` column to the `events` table (NULL for real, "migrator-v1_0_0-to-v1_0_1" for back-fill, "failure-detection-system-initiated" for system mints). Defer to a separate hardening story — non-blocking for Phase 2 correlation baseline. **Pass-3 UH-4 cross-ref**: the pass-2 ``X-Trace-Has-Synthetic`` response-header heuristic (TL-B14) was DROPPED in pass-3 because it relied on the same row-level inference (``row.trace_id == row.request_id``) and produced false +/- without test coverage. D6's per-row column is the proper fix.
- **D7 — `/trace` response shape vs canonical envelope** (pass-3 UM-1): The ``/trace`` response is a PRESENTATION view, not canonical replay. The ``extensions`` field is reserved but always empty until an ``extensions`` column is added to the Event ORM and materialised from ``envelope.extensions``. Operators needing byte-equivalent canonical output MUST consume the JSONL event log directly. Open a story to: (a) add the ORM column with non-NULL default; (b) wire the materializer to persist ``envelope.extensions``; (c) drop the always-empty placeholder in :mod:`registry_api.routes.trace`. Non-blocking for Phase 2 correlation baseline because no current consumer relies on round-trip canonical replay through ``/trace``.
