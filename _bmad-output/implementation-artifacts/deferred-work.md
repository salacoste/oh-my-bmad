...
- Per-server env scoping — ✅ CLOSED 2026-06-08. Story 43.1 implements per-server env filtering in MCPClientGroup._connect(). Each MCP child now receives only _BASE_ENV_VARS + its own server-specific vars. GITHUB_MCP_SCOPED_TOKEN no longer reaches non-github servers. Contract test test_per_server_env_isolation_github_scoped_token validates isolation. *Original:* Defense-in-depth; scoped token reaches all MCP children. Enhancement, not blocker. Revisit if fleet grows.
