...
| 2026-05-29 | Claude Opus 4.7 (1M ctx) via /code-review high | High-effort 7-angle review (3 correctness + 3 cleanup + 1 altitude). 10 batch-applied fixes: **R1** registry-api `/v1/health` response shape extended to satisfy `HealthResponseLocal` (4-field schema {registry_status, worker_status, clawhip_queue_depth, version}) — closes silent `/ping` UX regression (Cross-file C1, **CRITICAL**); **R2** "Webhook set · ready" log gated on skip-mode + dual signal "Webhook SKIPPED · lifespan ready" emitted on hermetic path — closes false-positive monitoring signal (Line-by-line B1, **CRITICAL**); **R3** `/v1/health` moved from inline `@app.get` to `routes/health.py` with `include_router(prefix="/v1")` — matches codebase convention (Altitude Alt2); **R4** `HERMETIC_WEBHOOK_URL` now composed from runtime `TELEGRAM_WEBHOOK_PATH` — closes path-coupling ValidationError (Line-by-line A1); **R5** `apply_hermetic_defaults_to_env` handles empty-string env-var (not just absent) via direct assignment after whitespace check — closes future `${VAR:-}` substitution gap (Line-by-line A2); **R6** lifespan-internal `apply_hermetic_defaults_to_env` call removed (dead code; production path is `__main__.py:212`) — closes Simplification S1 + ambiguity flagged by Edge-E1; **R7** truthy parse set aligned with pydantic-settings bool accept-set `{1,true,yes,y,on,t}` — closes Cross-file C4 divergence; **R8** Dockerfile `RUN chgrp -R + chmod -R` replaced by `COPY --chown=:omb --chmod=755` — image dropped 322MB→**286MB (-36MB)**, eliminated 5289-file recursive metadata-only overlayfs layer (Efficiency Eff1); **R9** `TELEGRAM_BOT_TOKEN: "0:dummytesttoken"` literal de-duplicated in s4.yml via `&dummy_telegram_token` YAML anchor (Reuse R3); **R10** `is_hermetic_skip_enabled_in_env()` inlined into single caller `apply_hermetic_defaults_to_env()` (Simplification S2). Refactor side-effect: registry-api `__version__` extracted to dedicated `_version.py` module so `routes/health.py` import doesn't trigger circular import via `app.py`. 842 tests pass in changed services; ruff/format/discipline clean; mypy 0-new on changed files. AC5 RAISED from "Partial / pending CI" to "CONFIRMED-by-shape" since the client-contract correctness is now provable from the typed `HealthResponse` schema in routes/health.py (Phase 1 healthcheck behaviour still requires CI Linux confirmation for the orch+worker MCP-init flake, but the spec's "registry-api serves /v1/health identically" is now satisfied with the correct shape). **Sprint-status flipped to `done`.** AC8 push + nightly remains user-authorised. |
