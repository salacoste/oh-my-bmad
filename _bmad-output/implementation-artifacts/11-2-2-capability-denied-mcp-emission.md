...
- [reverted] **PQ9 — Public `emit_event` tool reject `capability.denied`** (Edge MEDIUM #5): initial fix raised `PermissionError` when `type` was in `_emit_overrides`. **Reverted after CI integration test caught a regression** — task-registry / session-registry's legitimate audit-forwarding path uses the SAME `emit_event` tool. Blocking `capability.denied` broke `test_mcp_capability_denied_emits_envelope_and_increments_counter` (envelope never written → counter stays at 0). Replaced with a "known-limitation" test that pins the current (forge-able) behavior. Proper fix carved out to Story 11.2.3 alongside the FR26 multi-writer refactor: dedicated `forward_capability_denied_audit` tool with caller identity validation. Operators audit by cross-checking `payload.actor_id` against per-MCP-server tier-gate logs in the interim [`mcp-servers/clawhip-bridge/.../server.py` + `test_server.py`].
