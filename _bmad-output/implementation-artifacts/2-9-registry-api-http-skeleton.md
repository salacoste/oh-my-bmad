...
Generated by `/bmad-code-review` against scaffold commit `56e44b7`. Three parallel adversarial reviewers (Blind, Edge, Auditor — all opus). 25 actionable findings (10 CRITICAL, 4 MAJOR, 11 MINOR); 4 dismissed.

- [x] **[Review][Patch] POST→GET eventual-consistency race undocumented** [`routes/tasks.py` POST docstring + response shape] — **CRITICAL.** POST returns 201 with `task_id` immediately after `writer.append` finishes (event in JSONL); the materializer subscriber (separate process) hasn't applied the event to SQLite yet. An immediate `GET /v1/tasks/{task_id}` returns 404. Fix: add `Location: /v1/tasks/{task_id}` header to the 201 response; document the eventual-consistency contract in the POST handler docstring; add an integration-test comment noting the race.

- [x] **[Review][Patch] `writer.append` and `EventEnvelope.create` errors leak as text/plain 500** [`routes/tasks.py` POST + `app.py` exception_handler registrations] — **CRITICAL.** No `Exception` handler is registered; only `HTTPException` + `RequestValidationError`. Disk-full / fsync / `EventSchemaUnknown` (REGISTRY race) errors propagate as Starlette's default 500 with `text/plain`. Violates the AC-5 mandate that all 4xx/5xx use RFC 7807. Fix: register `app.add_exception_handler(Exception, handle_internal_error)` returning `application/problem+json` with status 500; log the original exception; sanitize the `detail` so internal stack traces don't leak.

- [x] **[Review][Patch] Validation error returns 400 instead of 422** [`adapters/errors.py` `handle_validation_error`] — **CRITICAL.** FastAPI/Pydantic v2 convention is 422 Unprocessable Entity for body validation failures. Current code: `ProblemDetails(title="Validation Error", status=400, ...)`. Breaks API conventions + OpenAPI consumers. Fix: status 422.

- [x] **[Review][Patch] Lifespan resource leak when `recover_all_logs` or `create_engine` raises** [`app.py:614-618`] — **CRITICAL.** Order: `writer = EventLogWriter(...)` → `await recover_all_logs(...)` → `engine = create_engine(...)`. If recover raises, the `writer` is constructed but never closed. If engine raises, both leak. Fix: use `AsyncExitStack` to register cleanup callbacks as resources are constructed:
  ```python
  async with AsyncExitStack() as stack:
      writer = EventLogWriter(...)
      stack.push_async_callback(writer.close)
      await recover_all_logs(base_dir)  # see F27 — may be removed
...
- [x] **[Review][Patch] `# type: ignore[arg-type]` in production code (AC-12 violation)** [`app.py:649-650`] — **CRITICAL.** Both `app.add_exception_handler(...)` calls carry `# type: ignore[arg-type]`. Completion Notes self-claim "no `# type: ignore` in production" is FALSE. Fix: type the handler signatures correctly. FastAPI 0.110+ accepts `Callable[[Request, Exception], Awaitable[Response]]`; if mypy still complains, the handlers should be defined with the broader signature OR FastAPI's `ExceptionHandler` type alias (if it exists in installed stub version).

- [x] **[Review][Patch] Empty `title=""` accepted as valid task** [`routes/tasks.py` `CreateTaskRequest`] — **CRITICAL.** `title: str` has no min_length. `{"title": ""}` produces 201 + permanently-uncorrectable empty-titled task in event log. Fix: `title: str = Field(min_length=1, max_length=512)`. Also apply to `TaskCreatedPayload` in `event_types.py` for defense in depth (the materializer trusts what's emitted).

- [x] **[Review][Patch] `X-Request-ID` header propagates unvalidated into event envelope** [`adapters/middleware.py` RequestIdMiddleware] — **CRITICAL.** Caller-supplied `X-Request-ID` flows directly to `request.state.request_id` → `EventEnvelope.create(request_id=...)` → JSONL log. Malicious or malformed input (`"\n\r"`, control chars, 10MB string) corrupts the canonical event log. Fix: validate against the bare-UUIDv7 regex `^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$` (Story 2.1). On invalid input, generate a fresh `new_request_id(clock)` and log a warning.

- [x] **[Review][Patch] `repo` and `hint` request fields are dead** [`routes/tasks.py` `CreateTaskRequest` + handler body] — **CRITICAL (contract).** Both fields are accepted via Pydantic + validated, then NEVER used by the handler — not added to `TaskCreatedPayload`, not stored anywhere. Future-compat trap: clients infer they're persisted. Fix: either (a) extend `TaskCreatedPayload` with `repo: str | None = None` + `hint: str | None = None` and wire them through, OR (b) drop both fields from `CreateTaskRequest`. Option (a) honors the spec's intent (FR1 mentions optional repo target + free-text hint).

- [x] **[Review][Patch] `session_maker` constructed per-request inside GET handler** [`routes/tasks.py:858`] — **CRITICAL (spec violation).** Spec AC-1 + sketch said "wires `async_sessionmaker`" + "stores `session_maker` on `app.state`". Current code: handler builds `async_sessionmaker(engine, expire_on_commit=False)` per request. Wasteful; spec compliance gap. Fix: lifespan creates `session_maker = async_sessionmaker(...)` once + stores on `app.state.session_maker`; handler reads `request.app.state.session_maker`.

- [x] **[Review][Patch] Duplicate Idempotency-Key creates duplicate tasks (no contract advertisement)** [`adapters/middleware.py` IdempotencyKeyMiddleware + `routes/tasks.py` POST response] — **CRITICAL (contract).** Story 2.7's cache exists but isn't wired (deferred to Story 3.6 per AC-12). Without dedup wired, two POSTs with same `Idempotency-Key` create distinct task rows. Spec is silent on advertisement. Fix: (a) add response header `X-Idempotency-Status: not-enforced` (Phase 1 marker — clients can read this and know dedup hasn't landed yet); (b) update OpenAPI route description to note the deferral with a Story 3.6 reference; (c) echo `Idempotency-Key` back in response headers so clients can correlate.

- [x] **[Review][Patch] `__main__.py` logs full DB URL — secret risk** [`__main__.py:297-303`] — **MAJOR.** Currently logs `db_url=%r`. SQLite path-only is benign today; Postgres in Phase 6 brings DSNs with passwords (`postgresql://user:pass@host/db`). Defense in depth. Fix: sanitize via `_redact_url(url)` helper that replaces password segment with `****`.

- [x] **[Review][Patch] No `base_dir.mkdir(parents=True, exist_ok=True)` on startup** [`app.py` lifespan] — **MAJOR.** First-run on a fresh container with non-existent `/var/lib/oh-my-bmad/registry/events` crashes with FileNotFoundError from `EventLogWriter.__init__` (which DOES `base_dir.mkdir(...)` per Story 2.4 AC-7). Wait — Story 2.4's writer ALREADY mkdir's. So this is auto-handled. But registry-api's lifespan calls `recover_all_logs(base_dir)` BEFORE constructing the writer; recover_all_logs runs `glob` which is safe on missing dir but the partial-tail trim opens for read. Actually Story 2.4's recover_all_logs returns 0 on missing dir. So no crash. **Demote to MINOR or dismiss.** Fix: verify by testing with non-existent `base_dir`; document the safety in lifespan docstring.

- [x] **[Review][Patch] Lifespan `engine.dispose()` blocked by `writer.close()` exception** [`app.py:91-92`] — **MAJOR.** If `writer.close()` raises (closed fd, EIO), `engine.dispose()` never runs → connection pool leak. Fix: subsumed by F5's `AsyncExitStack` refactor — each cleanup is registered independently and runs in reverse order regardless of exceptions in others.

- [x] **[Review][Patch] Missing 500-path test (AC-11 spec gap)** [`test_app.py` TestErrorHandlers] — **MAJOR.** Spec listed `test_unhandled_exception_returns_500_problem_json`; actual ships `test_unhandled_validation_returns_400_with_problem_json_envelope` — a duplicate of an existing 400-path test. 5xx envelope NEVER verified. Fix: add a real 500-path test that monkey-patches `EventLogWriter.append` to raise, asserts the response is 500 + `application/problem+json`. (Naturally verifies F2's exception handler too.)

- [x] **[Review][Patch] Empty body / invalid JSON tests missing** [`test_app.py` TestErrorHandlers] — **MINOR.** Add 2 tests: empty body (`content=b""`) → 422 problem+json; invalid JSON (`content=b"not-json"`) → 422 problem+json with appropriate detail.

- [x] **[Review][Patch] Uppercase `task_id` 422 test missing** [`test_app.py` TestGetTaskById] — **MINOR.** Add `test_get_task_with_uppercase_id_returns_422` — pass `T-019B...` to GET path; assert 422 problem+json (regex rejects uppercase).

- [x] **[Review][Patch] `_NEXT_COMMANDS.get(status, [])` silent for unknown status** [`routes/tasks.py:55`] — **MINOR.** Future materializer change (e.g., new status `"paused"`) returns `next_commands: []` — looks like terminal state but isn't. Fix: log a warning when status is not in `_NEXT_COMMANDS` keys; surfaces drift early.

- [x] **[Review][Patch] Echo `Idempotency-Key` in response header** [`adapters/middleware.py` IdempotencyKeyMiddleware] — **MINOR.** Companion to F11. Echoes the (possibly-generated) idempotency key back to the client for correlation. Fix: `response.headers["Idempotency-Key"] = request.state.idempotency_key` after `call_next`.

- [x] **[Review][Patch] `test_main_uses_default_host_port_when_env_absent` overrides db_url** [`test_app.py:1444-1479`] — **MINOR.** Test claims to test "when env absent" but sets `REGISTRY_API_DB_URL` to a tmp path. Misleading. Fix: rename to `test_main_uses_default_host_port_with_minimal_env`, or mock `os.path.exists` so the production-default DB path doesn't crash; assert default host/port are used.

- [x] **[Review][Patch] Production handler rolls own sessionmaker; test imports `get_session`** [`routes/tasks.py:858` + `test_app.py:962`] — **MINOR.** Inconsistent. Fix: lifespan uses `get_session(engine)` from Story 2.3 (matches Story 2.5/2.6 pattern); store result on `app.state.session_maker`. Combines with F10.

- [x] **[Review][Patch] `f"Task {task_id!r} not found"` in 404 detail** [`routes/tasks.py:867`] — **MINOR.** `!r` adds quotes around the id. Cosmetic but inconsistent with structured envelope philosophy. Fix: drop the `!r`: `f"Task {task_id} not found"`.

- [x] **[Review][Patch] `__all__` exceeds spec** [`__init__.py:223`] — **MINOR.** Spec said `__all__ = ["build_app"]`. Actual: `["build_app", "hello", "__version__"]`. Trim to `["build_app"]` (remove `hello` from public surface; legacy scaffold-only); leave `__version__` since it's idiomatic.

- [x] **[Review][Patch] `recover_all_logs` race between registry-api + registry-state lifespans** [`app.py:76`] — **MINOR.** registry-api calls recovery defensively; registry-state subscriber also calls it. Both are idempotent at the file level (Story 2.4) but no cross-process lock. Phase 1 deployment runs them in separate compose services with staggered startup; race is theoretical. Fix: drop the recovery call from registry-api's lifespan — let the materializer (registry-state) own it. Document in app.py docstring: "recover_all_logs is registry-state's responsibility; registry-api is a read-mostly + append-only-events service."

- [x] **[Review][Patch] Dual-source `actor_id` (app.state vs request.state)** [`app.py:85` + `adapters/middleware.py` ActorIdMiddleware + `routes/tasks.py` handler] — **MINOR.** Both places set actor_id. Handler reads `app.state.actor_id`; spec implies it should be `request.state.actor_id`. Pick one source. Fix: drop `app.state.actor_id` mirror; handler reads `request.state.actor_id` (set by ActorIdMiddleware on every request).

- [x] **[Review][Patch] Middleware ordering test missing** [`test_app.py` TestMiddleware] — **MINOR.** AC-4 mandates ordering RequestId → Idempotency → ActorId (outer-to-inner). No test asserts this. Fix: add `test_middleware_ordering` — uses a debug endpoint that records the order of `request.state` mutations during request processing; asserts request_id is set first, idempotency_key second, actor_id third.

Dismissed (documented for auditability):

- `0.0.0.0` default bind — Docker compose convention; security per NFR-S7 (compose-network only).
- `/openapi.json` + `/docs` exposed — Phase 1 internal-only per NFR-S7; flag for future story.
- bootstrap-verify slowdown from FastAPI eager import — Phase 1 acceptable; <1s on dev hardware.
- Test debug endpoint registered after build_app — test-only; doesn't affect production middleware semantics.
...
| 2026-04-25 | 1.1 | Code review — 3 parallel adversarial reviewers (Auditor flagged AC-12 type:ignore + AC-11 missing 500-path test; Blind + Edge surfaced 23 more). 25 actionable findings (10 CRITICAL, 4 MAJOR, 11 MINOR) all fixed; 4 dismissed. CRITICAL: POST→GET eventual-consistency contract (Location header + docstring); generic Exception → problem+json 500 (no more text/plain leakage); validation 400 → 422; AsyncExitStack lifespan (resource leak fix); type:ignore removed (handler signatures use Exception + isinstance narrowing); title min_length=1 (no empty-titled tasks); X-Request-ID UUIDv7 validation (eliminates log-injection vector); repo/hint wired through TaskCreatedPayload (no more dead fields); session_maker on app.state (per spec); Idempotency advertisement headers (X-Idempotency-Status: not-enforced + key echo). MAJOR: _redact_url for DB credentials in logs; mkdir behavior verified + tested; 500-path test. MINOR: empty body / invalid JSON / uppercase task_id tests; warning on unknown status; test rename; 404 without !r; __all__ trim; request.state.actor_id only; middleware ordering test. +11 net tests (386+6 → **397+6**). mypy --strict still clean on 62 files (NOW zero `# type: ignore` in production). 9 empirical probes all PASSED. Fix commit: `38d0744`. Status → done. |
