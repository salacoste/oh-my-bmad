(function () {
  ...
  const ROUTE_PATTERN = "GET /v1/tasks?limit={task_list_limit}&offset={task_list_offset}";
  const MAX_LIMIT = 50;
  ...
  function readSelectors() {
    ...
    if (!limitControl || !offsetControl) return null;
    if (limitControl.type === "hidden" || offsetControl.type === "hidden") return null;
  ...
  function selectedRoute(selectors) {
    return `${ROUTE}?limit=${selectors.limit}&offset=${selectors.offset}`;
  }
  ...
  function renderBody(body, selectors) {
    ...
    if (state !== "healthy") {
      render(state, "non-authoritative", body.retrieved_at, provenance, correlation, String(body.selected_limit), String(body.selected_offset), runtimeRoute, pagination, state, String(body.returned_count), `${state} aggregate task list limit and offset response; not authoritative.`);
      return;
    }
    render(state, authority, body.retrieved_at, provenance, correlation, String(body.selected_limit), String(body.selected_offset), runtimeRoute, pagination, "none", String(body.returned_count), body.items.map(rowText).join("\n"));
  }
