"""Canonical event-type payload re-exports and schema registrations.

This shared module owns the platform-wide ``ensure_registered()`` hook so every
runtime that validates envelopes can install the same canonical schema set
without importing a service package. ``registry_state.domain.event_types`` is
kept as a compatibility shim that re-exports this module.
"""
