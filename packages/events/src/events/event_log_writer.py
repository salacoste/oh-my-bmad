...
Relocated from ``services/registry-state/src/registry_state/adapters/event_log.py``
so that cross-service consumers (registry-api, telegram-gateway, clawhip-bridge)
can import from the shared ``events`` package without cross-service import
violations.
"""
...
from events.log_reader import current_day_path
