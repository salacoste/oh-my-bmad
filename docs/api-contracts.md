...
| `/v1/tasks` | GET | `get_tasks` | Bounded aggregate task summary list. Selector-free/body-free fixed first page ordered by `updated_at DESC, id ASC`; exposes source/freshness/provenance metadata and no task-detail/session/digest/history/trace traversal. Story 113.2 additionally permits exactly `GET /v1/tasks?status={task_status}` with one finite status query key. Story 114.2 additionally permits exactly `GET /v1/tasks?limit={task_list_limit}` with one integer `limit` query key from 1 through 50. Story 115.2 additionally permits exactly canonical-order `GET /v1/tasks?status={task_status}&limit={task_list_limit}`. Story 117.2 additionally permits exactly canonical-order `GET /v1/tasks?limit={task_list_limit}&offset={task_list_offset}` with offset `0..2147483647`. Story 120.2 additionally permits exactly canonical-order `GET /v1/tasks?status={task_status}&limit={task_list_limit}&offset={task_list_offset}` with status filtering before offset windowing. Story 124.2 permits standalone API-local `GET /v1/tasks?sort={task_sort}` with exactly `updated_at_desc_id_asc` and `created_at_desc_id_asc`, selected_sort metadata, and deterministic per-token ordering. Story 125.2 additionally permits exactly canonical-order API-local `GET /v1/tasks?status={task_status}&limit={task_list_limit}&offset={task_list_offset}&sort={task_sort}`; filtering occurs before selected sort ordering and offset windowing, response metadata includes selected status/limit/offset/sort plus bounded pagination metadata, and accepted sort values remain exactly the Story 124.2 finite vocabulary. Story 127.2 additionally permits API-local bodyless search `GET /v1/tasks?field={task_search_field}&op={task_search_operator}&q={task_search_query}` with only explicitly registered suffixes after `q`: no suffix, `status`, `limit`, `status&limit`, `limit&offset`, `status&limit&offset`, `sort`, or `status&limit&offset&sort`; response metadata includes selected field/op/query, selected suffixes, redaction state, and existing freshness/authority/provenance/request/trace/correlation/pagination metadata. Reversed query order, extra/repeated query keys, encoded keys/values, GET bodies, status+offset without limit, partial sort composition and browser sort composition outside the shipped Phase 47 / Story 126.2 visible aggregate-task-list full-composition browser consumption boundary, unregistered search grammar, cursor/page traversal beyond the exact limit+offset API-local boundary, hidden selectors, traversal, replay execution, lifecycle mutation, generated live data, and broad dashboard wiring remain rejected/deferred. |
...
Story 127.2 implements the API-local runtime boundary. Search remains route-local to the aggregate list as bodyless `GET /v1/tasks?field={task_search_field}&op={task_search_operator}&q={task_search_query}` followed only by optional existing selectors in canonical order: `status`, `limit`, `offset`, `sort`.

Runtime search fields/operators are exactly `task_id:eq`, `title:contains|prefix`, `status:eq`, `actor_id:eq|prefix`, `last_event_type:eq`, `updated_at:gte|lte`, and `created_at:gte|lte`. Global `q` is required, raw ASCII-only, `1..96` bytes, with per-field caps `task_id/title/actor_id 1..64`, `last_event_type 1..80`, and timestamp fields exactly 20 chars in UTC `YYYY-MM-DDTHH:MM:SSZ`; full raw query strings are capped at `1..256` bytes. Percent encoding / percent-encoded bytes, `+`, raw spaces, controls, Unicode/non-ASCII, repeated/encoded keys, aliases, reordered keys, empty values, GET bodies, boolean DSL, regex, fuzzy search, SQL-like syntax, wildcards, nesting, multiple search fields, and arbitrary JSON remain rejected. `field=status` with a separate `status=` selector is fail-closed duplicate status semantics.

Story 127.3 implements browser search for the aggregate task-list panel from visible `field`, `op`, `q`, `status`, `limit`, `offset`, and `sort` controls only. The search button composes raw validated query text and issues exactly one bodyless `GET /v1/tasks?field={task_search_field}&op={task_search_operator}&q={task_search_query}&status={task_status}&limit={task_list_limit}&offset={task_list_offset}&sort={task_sort}` with `credentials: "omit"`; page load and selector edits do not auto-run search. Browser-side `field=status` remains intentionally unavailable to avoid duplicate `field=status&status=` composition. URL/hash/storage/cookie, hidden input, row-derived, server-provided route string, and background-derived selectors remain forbidden and fail closed before a search fetch. Search results do not authorize automatic traversal, prefetch, infinite scroll, timers, workers, observers, retry loops, cache warming, websocket/EventSource/XMLHttpRequest side channels, automatic next-page reads, or manual previous/next traversal from search pagination metadata.
...
- Epic 129 implements approval-bound reversible `prune_hot_segment` apply/rollback. Apply is distinct from dry-run and is bound to durable evidence for the exact dry-run `plan_hash`, affected segment identities, current replay validation, rollback evidence, expiry-bound approval, and explicit operator identity/event or ledger reference; absent, failed, stale, ambiguous, or unverifiable evidence blocks apply. Unsupported destructive classes (hard delete, truncate, arbitrary move/rewrite/chmod, archive or manifest mutation, object-storage deletion, scheduled retention, production credentials, and cross-service destructive operations) remain fail-closed.
