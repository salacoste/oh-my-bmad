...
- **Phase 23 closed the task-detail runtime boundary.** Story 102.1 selected exactly `GET /v1/tasks/{task_id}`, Story 102.2 implemented the narrow task-detail browser runtime boundary, and Story 102.3 recorded final closure evidence.
- **Phase 24 closed Event timeline / transitions runtime boundaries.** Phase 24 selected and completed exact task-scoped read routes `GET /v1/tasks/{task_id}/events` and `GET /v1/tasks/{task_id}/transitions` without broad dashboard wiring.
- **Phase 25 closed Trace correlation runtime boundary.** Phase 25 selected and completed exact `GET /v1/trace/{trace_id}` without discovery or mutation/control expansion.
- **Phase 26 closed History / Replay runtime boundary.** Phase 26 selected and completed exact `GET /v1/tasks/{task_id}/history`, `GET /v1/events/replay`, and `GET /v1/events/replay/validate` with visible replay target query discipline.
- **Phase 27 closed Lifecycle / Snapshot runtime boundary.** Phase 27 selected and completed exact `GET /v1/events/replay/snapshots` plus passive lifecycle-readiness evidence display; Story 106.3 records final Phase 27 / Epic 106 closure after remote CI run [`28139358221`](https://github.com/salacoste/oh-my-bmad/actions/runs/28139358221).
- **Phase 28 closed Snapshot Creation authorization boundary.** Phase 28 selected and completed exact JWT-authenticated `POST /v1/events/replay/snapshots` through a visible operator affordance; Story 107.3 records final Phase 28 / Epic 107 closure after remote CI run [`28195545005`](https://github.com/salacoste/oh-my-bmad/actions/runs/28195545005).
- **Dashboard runtime remains intentionally narrow.** Approved browser runtime is limited to health/readiness, task detail, event timeline/transitions, trace correlation, history/replay, lifecycle/snapshot listing, and the single visible JWT-authenticated snapshot-create affordance. Deferred/fail-closed/unauthorized boundaries remain: aggregate/session/digest; task-list/search/discovery; broad dashboard wiring; lifecycle apply/prune/rollback; destructive lifecycle implementation; archive/manifest mutation; snapshot deletion/restore; additional mutation/control affordances; services/MCP/dependencies/CI changes.

The following items remain unshipped or intentionally out of scope after Phase 28 closure:
...
- **Broad web dashboard runtime wiring** -- browser-based operator surface exists as a static shell with narrow approved health, task detail, event timeline/transitions, trace correlation, history/replay, lifecycle/snapshot listing, and snapshot-create authorization boundaries; broader route families and broad dashboard wiring remain deferred/fail-closed and require future per-route approval.
