...
- **Current phase:** Phase 34 — open after planning selection. Story 113.1 is done as docs/status-only route selection for future read-only task status filter boundary `GET /v1/tasks?status={task_status}` after Architect APPROVE/CLEAR followed by Critic APPROVE/CLEAR; runtime/API/browser/test implementation remains unauthorized until a later tests-first Story 113.2.
- **Current epic:** Epic 113 is in progress after Story 113.1 task status filter route-selection planning; Epic 112 is done after Story 112.3 final closure and remote CI run `28291210521`.
- **Recently closed:** Phase 33 / Epic 112 — Digest stream runtime/API boundary and final validation closure are done.
- **Open in Phase 34:**
  - Story 113.1 selects the read-only task-list/search/discovery family and narrows the exact future candidate to `GET /v1/tasks?status={task_status}` with one lifecycle status selector only (`pending`, `planning`, `plan_ready`, `executing`, `blocked`, `completed`, `stopped`, or `failed`). Architect APPROVE/CLEAR and subsequent Critic APPROVE/CLEAR are recorded.
  - Story 113.1 is complete as planning/status-only: current `GET /v1/tasks` query rejection remains unchanged, and no runtime/API/browser/test behavior is implemented or authorized.
  - Story 113.2 / 113.3 remain backlog. Free-text search, arbitrary discovery, pagination/sort controls, hidden selectors, row-driven traversal, replay execution target selection, lifecycle apply/prune/rollback, broad dashboard wiring, generated live data, browser-side LLM behavior, mutation/control behavior, services/MCP/dependencies/CI/deployment changes, production credentials, and production operations remain deferred/fail-closed.
- **Closed in Phase 33:**
...
- **Not closed by this page:** Story 113.1 status-filter route planning is docs/status-only complete; runtime/API implementation for `GET /v1/tasks?status={task_status}`, lifecycle apply/prune/rollback, destructive lifecycle authorization execution, archive/manifest mutation, snapshot deletion/restore, free-text task search/discovery, generated live data, replay execution target selection, broad dashboard live wiring, backend/API expansion beyond exact approved dashboard runtime routes, additional controls, services, MCP changes, deployment changes, dependencies, lockfiles, production operations, and unapproved surfaces beyond the exact status-filter candidate remain deferred/fail-closed.
...
- `../_bmad-output/implementation-artifacts/sprint-status.yaml` — `current_phase: 34`, `epic-113` in progress, `113-1` done, `113-2`/`113-3` backlog, and historical `epic-112` done with remote CI run `28291210521` recorded.
