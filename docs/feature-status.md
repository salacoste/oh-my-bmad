...
- **Current phase:** Phase 32 — session-detail runtime/API boundary is locally implemented for exactly `GET /v1/sessions/{session_id}`; Story 111.2 is in review with code-review/UltraQA clear, and Story 111.3 remains backlog until push and remote CI evidence exist.
...
- **In review:** Phase 32 / Epic 111 — Story 111.2 locally implements exact `GET /v1/sessions/{session_id}` after Story 111.1 route-selection planning; final completion awaits push and remote CI evidence.
- **Done in Phase 27:**
...
- `../_bmad-output/implementation-artifacts/111-2-session-detail-runtime-boundary.md` — Story 111.2 local tests-first runtime/API implementation, code-review/UltraQA, and validation evidence, pending push/remote-CI final closure.
...
| Aggregate/session dashboard live contracts | Aggregate task list and session list implemented with recorded closure; session detail locally implemented and in review | Phase 30 / Story 109.2 implements exactly `GET /v1/tasks` for bounded aggregate task summaries and Story 109.3 records green remote CI run `28213044828`. Phase 31 / Story 110.2 implements exactly `GET /v1/sessions` after code-review/architect/UltraQA gates and Story 110.3 records green remote CI run `28248851773`. Phase 32 / Story 111.2 locally implements exact `GET /v1/sessions/{session_id}` with a visible session_id source, query/body rejection, bounded Session-table-only metadata, and fail-closed rendering; final completion awaits push/remote CI. Digest streaming, search/discovery, hidden selectors, automatic drill-down, and broad dashboard wiring remain unavailable/needs-contract until separately approved. |
