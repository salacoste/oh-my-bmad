...
- **Current phase:** Phase 33 — local implementation evidence exists for Story 112.2 exact digest-stream runtime/API boundary; Story 112.3 local closure is prepared but remains remote-CI/push-evidence pending.
- **Current epic:** Epic 112 is in progress after Story 112.1 planning completion and local Story 112.2 implementation; Epic 111 remains done after Story 111.3 final closure and remote CI run `28259115072`.
- **Recently closed:** Phase 32 / Epic 111 — Session detail runtime/API boundary and final validation closure are done.
- **Open in Phase 33:**
  ...
  - Story 112.3 remains final-closure pending until code-review/QA artifacts are complete and push/remote CI evidence exists; this page does not claim remote CI for Story 112.2.
- **Done in Phase 27:**
...
- **Not closed by this page:** lifecycle apply/prune/rollback, destructive lifecycle authorization execution, archive/manifest mutation, snapshot deletion/restore, task-list/search/discovery beyond exact approved reads, generated live data, replay execution target selection, broad dashboard live wiring, backend/API expansion beyond exact approved dashboard runtime routes, additional controls, services, MCP changes, deployment changes, dependencies, lockfiles, production operations, and Story 112.2 push/remote-CI finalization.

Primary evidence:

- `../_bmad-output/implementation-artifacts/sprint-status.yaml` — `current_phase: 33`, `epic-112` in progress, `112-1` done as docs/status-only route-selection planning after repaired Architect/Critic consensus, `112-2` local implementation in review, and `112-3` local closure prepared but final push/remote-CI evidence pending.
...
| Digest stream dashboard runtime | Locally implemented / remote-CI pending | Phase 33 / Story 112.2 implements exactly `GET /v1/tasks/{task_id}/logs/digest/stream` with API `application/x-ndjson` frames (`open`, `chunk`, `final`) and additive dashboard `digest-stream.js` using visible `task_id` text only. The non-streaming digest route remains independent; task-list/search/discovery, fallback traversal, generated browser summaries, EventSource/WebSocket/XMLHttpRequest/workers/polling/retry/storage, mutation/control behavior, services/MCP/dependencies/CI/deployment changes, production credentials, and production operations remain unavailable. Local tests are recorded in the Story 112.2 artifact; remote CI is not yet claimed. |
