...
> named below on 2026-06-27. If this page conflicts with sprint-status, trust
> sprint-status and update this derivative matrix.

## Current BMad status

- **Current phase:** Phase 34 — closed after Story 113.3 final validation closure. Story 113.2 implements/proves exactly `GET /v1/tasks?status={task_status}` as a tests-first API/runtime boundary with one finite lifecycle status selector only; implementation commit `32fdbaea23816df72bd999b9eb992bab7262ab43` is proven by GitHub Actions `ci` run `28298018048` success.
- **Current epic:** Epic 113 is done after Story 113.3 final closure; Epic 112 is done after Story 112.3 final closure and remote CI run `28291210521`.
- **Recently closed:** Phase 34 / Epic 113 — Task status filter runtime/API boundary and final validation closure are done. Phase 33 / Epic 112 — Digest stream runtime/API boundary and final validation closure are done.
- **Closed in Phase 34:**
  ...
  - Story 113.3 records final closure with implementation commit `32fdbaea23816df72bd999b9eb992bab7262ab43` and GitHub Actions `ci` run `28298018048` success. Free-text search, arbitrary discovery, pagination/sort controls, hidden selectors, row-driven traversal, replay execution target selection, lifecycle apply/prune/rollback, broad dashboard wiring, generated live data, browser-side LLM behavior, mutation/control behavior, services/MCP/dependencies/CI/deployment changes, production credentials, and production operations remain deferred/fail-closed.
- **Closed in Phase 33:**
...
- **Not closed by this page:** lifecycle apply/prune/rollback, destructive lifecycle authorization execution, archive/manifest mutation, snapshot deletion/restore, free-text task search/discovery, generated live data, replay execution target selection, broad dashboard live wiring, backend/API expansion beyond the exact approved status-filter route, additional controls, services, MCP changes, deployment changes, dependencies, lockfiles, production operations, and unapproved surfaces beyond the exact status-filter candidate remain deferred/fail-closed.
...
- `../_bmad-output/implementation-artifacts/sprint-status.yaml` — `current_phase: 34`, `epic-113` done, `113-1`/`113-2`/`113-3` done, and historical `epic-112` done with remote CI run `28291210521` recorded.
