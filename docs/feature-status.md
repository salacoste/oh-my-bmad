...
- **Current phase:** Phase 30 — closed for the aggregate task list runtime/API boundary, exactly `GET /v1/tasks`, with Story 109.3 remote CI evidence (`ci` run `28213044828`, https://github.com/salacoste/oh-my-bmad/actions/runs/28213044828).
- **Current epic:** Epic 109 is done after Story 109.1 selected exactly `GET /v1/tasks`, Story 109.2 implemented the exact aggregate task list runtime/API boundary with code-review APPROVE, architect recheck CLEAR, UltraQA PASS, local PR-gate-equivalent validation, and Story 109.3 recorded push plus green remote CI run `28213044828` (https://github.com/salacoste/oh-my-bmad/actions/runs/28213044828).
...
- `../_bmad-output/implementation-artifacts/sprint-status.yaml` — `current_phase: 30`, `epic-108` done, `epic-109` done, and `109-1`/`109-2`/`109-3` done with remote CI run `28213044828` recorded for closure.
...
| Aggregate/session dashboard live contracts | Aggregate task list implemented with recorded closure; sessions fail-closed | Phase 30 / Story 109.2 implements exactly `GET /v1/tasks` for bounded aggregate task summaries and Story 109.3 records green remote CI run `28213044828`. `/v1/sessions`, `/v1/sessions/{session_id}`, digest streaming, search/discovery, hidden selectors, automatic drill-down, and broad dashboard wiring remain unavailable/needs-contract until separately approved. |
