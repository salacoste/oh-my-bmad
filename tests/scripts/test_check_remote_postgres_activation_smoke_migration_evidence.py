...
def test_story_134_6_planning_closure_status_is_allowed(tmp_path: Path) -> None:
    ...
    feature.write_text(
        feature.read_text(encoding="utf-8").replace(
            ...
            "remote Postgres smoke/migration evidence",
        )
        + "\nEpic 134 controlled production activation evidence planning is done as planning-only/docs-status closure, not activation.\n",
        encoding="utf-8",
    )
    violations = mod.validate(tmp_path)  # type: ignore[attr-defined]
