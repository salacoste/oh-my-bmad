"""verification-mcp entrypoint — ``python -m verification_mcp`` (Epic 17 / Story 17.2).
...
---------------------
VERIFICATION_MCP_WORKTREE_ROOT  (REQUIRED)
    Absolute path to the sandbox worktree root. All verification recipes (Stories
    17.3 / 17.4) are confined to this directory. Missing or empty → exit 2.

VERIFICATION_MCP_ACTOR_KIND  (REQUIRED)
    One of ``operator|orchestrator|worker|system|clawhip``.
    Missing/empty/invalid → exit 2.

VERIFICATION_MCP_ACTOR_ID  (REQUIRED)
...
OMB_MCP_AUDIT_EMISSION_ENABLED  (OPTIONAL, default ``1``)
    ...
    ``0`` to disable explicitly; legacy
    ``VERIFICATION_MCP_DISABLE_AUDIT_EMISSION=1`` is the operator kill-switch.

VERIFICATION_MCP_CLAWHIP_BRIDGE_COMMAND  (OPTIONAL, default ``sys.executable``)
    Command to spawn the clawhip-bridge MCP subprocess for audit emission.
    Only consulted when audit emission is enabled.

VERIFICATION_MCP_CLAWHIP_BRIDGE_ARGS  (OPTIONAL, default ``-m clawhip_bridge_mcp``)
...
VERIFICATION_MCP_DISABLE_AUDIT_EMISSION  (OPTIONAL, default ``0``)
...
REGISTRY_EVENTS_DIR  (OPTIONAL)
    Base dir of the JSONL event log (reserved for the ``verification.*`` event
    emission in Story 17.4). When unset, no event source is threaded.
...
def main() -> None:
    ...
    # -- Required: VERIFICATION_MCP_WORKTREE_ROOT --
    worktree_root_raw = os.environ.get("VERIFICATION_MCP_WORKTREE_ROOT", "").strip()
    if not worktree_root_raw:
        print(
            "verification-mcp: VERIFICATION_MCP_WORKTREE_ROOT is required but not set or empty.",
            file=sys.stderr,
        )
        sys.exit(2)

    # -- Required: VERIFICATION_MCP_ACTOR_KIND --
    actor_kind_raw = os.environ.get("VERIFICATION_MCP_ACTOR_KIND", "").strip()
    if not actor_kind_raw:
        print(
            "verification-mcp: VERIFICATION_MCP_ACTOR_KIND is required but not set or empty. "
    ...
    if actor_kind_raw == "operator":
        ...
        print(
            f"verification-mcp: VERIFICATION_MCP_ACTOR_KIND={actor_kind_raw!r} is invalid. "
            f"Must be one of: {', '.join(sorted(_VALID_ACTOR_KINDS))}",
            file=sys.stderr,
        )
        sys.exit(2)

    # -- Required: VERIFICATION_MCP_ACTOR_ID --
    actor_id = os.environ.get("VERIFICATION_MCP_ACTOR_ID", "").strip()
    if not actor_id:
        print(
            "verification-mcp: VERIFICATION_MCP_ACTOR_ID is required but not set or empty.",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    # Legacy ``VERIFICATION_MCP_DISABLE_AUDIT_EMISSION=1`` force-disables regardless.
    enable_audit = os.environ.get("OMB_MCP_AUDIT_EMISSION_ENABLED", "1").strip() != "0"
    force_disable_audit = (
        os.environ.get("VERIFICATION_MCP_DISABLE_AUDIT_EMISSION", "").strip() == "1"
    )
    clawhip_cmd: str | None = None
    clawhip_args: list[str] | None = None
    if enable_audit and not force_disable_audit:
        clawhip_cmd = (
            os.environ.get("VERIFICATION_MCP_CLAWHIP_BRIDGE_COMMAND", "").strip()
            or sys.executable  # use sys.executable, not bare "python"
        )
        clawhip_args_raw = (
            os.environ.get("VERIFICATION_MCP_CLAWHIP_BRIDGE_ARGS", "").strip()
            or "-m clawhip_bridge_mcp"
        )
        # shlex.split handles quoted paths with spaces.
        clawhip_args = shlex.split(clawhip_args_raw)

    # -- Optional: REGISTRY_EVENTS_DIR (Story 17.4 verification.* event source) --
    # The base dir of the JSONL event log, reserved for the ``verification.*``
    # event emission (Story 17.4). Threaded for forward-compat with the event
    # surface; the verification tools are Tier-2 (no approval gate) so it is not
    # consulted for an approval lookup.
