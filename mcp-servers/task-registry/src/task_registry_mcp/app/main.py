...
from task_registry_mcp.adapters.clawhip_client import ClawhipBridgeClient, EmitterHolder
    ...
    *,
    ...
    if clawhip_bridge_command is not None and clawhip_bridge_args is not None:
        ...
        async def _lifespan(_server: FastMCP) -> AsyncGenerator[None, None]:
            ...
            PD-1 fail-soft mid-request is owned by the decorator; startup
            failure (e.g., clawhip-bridge subprocess refuses to launch)
            ...
            log.info(
                "task_registry_clawhip_client_ready",
    ...
    mcp = (
        FastMCP("task-registry", lifespan=lifespan_fn) if lifespan_fn else FastMCP("task-registry")
    )
